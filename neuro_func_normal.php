...
function train()
{
    ...
    for ($k = 0; $k < count($allinput); ++$k) {
        ...
        $error = myself($output_layer["data"] - ($allinput[$k][1]/10));
...
function check_all()
{
    ...
    foreach ($allinput as $current) {
        $input = $current[0];
        calculate();
        echo $input . " expected: " . $current[1] . " got: " . $output_layer["data"]*10 . PHP_EOL;
    }
}
